...
$years = [];
for ($y = 2026; $y >= 1996; $y--) {
    $stmt = $pdo->prepare("SELECT g.id, g.title, g.resource_size, g.cover_image_local, g.release_date FROM bo_game g WHERE YEAR(g.release_date) = :year ORDER BY g.score DESC, g.id DESC LIMIT 10");
    $stmt->execute([':year' => $y]);
