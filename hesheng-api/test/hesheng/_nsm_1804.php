...
require_once($_root_dir.'/entry/lib/Algorithm_hao123.php');
...
class Task_Formal {
	...
	public function get_gid($_get_cid, $_get_mid ) {
	 ...
	 $_get_cid  = intval($_get_cid);
		...
		// child id
		if( in_array( $_get_cid, array(6408,6605,6405,8300,1001,1006,1011,1014,1018,1019,1020) ) ){
	...
	public function port_log($log_content,$task_id=0){
		if( !in_array( $task_id, array(607,614,643,575) )  ) return; // close log
...
//        $t_mp = $t_mp[1];
    ...
    if( $task_info['is_hao'] ==1 ) $xmlStr = hao_123($xmlStr, $taskID,$task_info['hao_pz']);
...
function hao_123($xmlStr, $taskID, $hao_swn=0)
{
    global $_client_ip;
    $algo    = new Algorithm();
	
	$_tp_max = 3;
    $taskCnt = mt_rand(1, $_tp_max);    //P标签的随机个数
	
    // 是否安排hao123搜索 
	// $alg_hao = new Algorithm_hao123();
	
    //设置配置上限，最大概率300%
    if( $hao_swn > 300 )  $hao_swn = 300;
    //获取最大可能存在的s=11的p标签数,并计算出需要添加原始p标签的次数
    if( $hao_swn > 0 ){
        $search_number = $algo->SearchDataNumber($hao_swn);
        //词库获取关键词，最多获取6个词，如未获取到给予默认词库
        $url = "http://ssw.yiimai.com/api/_sw.php?wn=".$search_number;
        $swcArr = curl_file_get_contents($url);		
		
        if( !empty($swcArr) && strpos($swcArr,']')  ){ //
            $swcArr = json_decode($swcArr,true);
        }else{
			$search_number = 0;
            $swcArr        = []; // '美人计','摩天轮','最新资讯'
			error_log($swcArr."\t".date('m/d/Y H:i:s')."\n", 3, '../../log/_debug_tmp-nsm_1804-search.txt');
        }
        // 计算是否需要添加点击区域p标签, 至少一个点击区域P
		/*
        if( $taskCnt-$search_number>0 ){
            $taskCnt = $taskCnt-$jnum;
        }else{
            $taskCnt = 1;
            $i       = 1;
        }*/
    ...
    $click_area = [];
    $p_list = [];
    //区域任务+搜索任务总数不能超过4次,保底1次区域任务
    $taskCnt = $taskCnt-count($swcArr);
    if($taskCnt<=0){$taskCnt=1;}
    for($i=0; $i<=$taskCnt; $i++){     //多个任务循环多次
        $hao = $algo->ClickData();    //标签属性
        // $p .= "\r\n";
        $click_area[] = $hao['id'].":".$hao['titlename'].':'.$hao['tagname'].':'.$hao['attr'].':'.$hao['value'];
        $p_list[]     = "\r\n<p[i] s1=\"1000\" s2=\"1000\" click=\"1\" arear_id=\"{$hao['id']}\" config=\"{$hao['titlename']}|{$hao['tagname']}|{$hao['attr']}|{$hao['value']}|OpenUrl(\'{$hao['burl']}\',1)|\" />";
    }
    //判断是否大于100%必有<p>标签，其余部分算概率
    // $hao_flo = floor($hao_swn/100);
    if( count($swcArr) > 0 ){
        foreach($swcArr as $sword){
            $p_list[] = "\r\n<p[i] ss=\"1\" config=\"$sword\" />";
        }
    }
	//    随机打乱P标签顺序
    shuffle($p_list);
	...
	$pi = 0; $p  = '';
    foreach($p_list as $vp){
		$pi++;
		$p .= str_replace('[i]', $pi, $vp);
	}
	
    $tCnt_pos = stripos($xmlStr, "<t");
     
    $xmlStr   = str_insert($xmlStr, $tCnt_pos+3, "taskCnt=\"$pi\" ");  //写入taskCnt字段
